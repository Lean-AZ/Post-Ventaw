...
class srTenancyAgreement(models.Model):
    ...
    def _compute_amount_all(self):
        for order in self:
            ...
            difference = relativedelta(
                order.agreement_expiry_date, order.agreement_start_date
            )
            if (
                order.agreement_start_date
                and order.agreement_duration
                and order.agreement_duration_type
            ):
                if order.agreement_duration_type == "month":
                    order.agreement_expiry_date = (
                        order.agreement_start_date
                        + relativedelta(months=order.agreement_duration)
                    )
                else:
                    order.agreement_expiry_date = (
                        order.agreement_start_date
                        + relativedelta(years=order.agreement_duration)
                    )
                num_months = (
                    order.agreement_expiry_date.year - order.agreement_start_date.year
                ) * 12 + (
                    order.agreement_expiry_date.month - order.agreement_start_date.month
                )
                difference = relativedelta(
                    order.agreement_expiry_date, order.agreement_start_date
                )
            ...
            if order.property_id.property_type == "rent":
                ...
                if order.commission_type == "percentage":
                    ...
                    ) * (order.agent_commission / 100)
                else:
                    commission = order.agent_commission
                if order.maintenance_interval_type == "month":
                    maintenance_charge = (
                        order.property_id.property_maintenance_charge * num_months
                    )
                else:
                    if difference.years > 1:
                        maintenance_charge = (
                            order.property_id.property_maintenance_charge
                            * difference.years
                        )
                    else:
                        maintenance_charge = (
                            order.property_id.property_maintenance_charge * 1
                        )
                ...
                if order.commission_type == "percentage":
                    commission = (order.property_sale_price) * (
                        order.agent_commission / 100
                    )
                else:
                    commission = order.agent_commission
    ...
    @api.onchange(
        "agreement_start_date", "agreement_duration", "agreement_duration_type"
    )
    ...
    name = fields.Char(
        string="Tenant Agreement Reference",
        required=True,
        copy=False,
        readonly=True,
        index=True,
    ...
    agreement_date = fields.Date(
        ...
        index=True,
        tracking=1,
        help="Creation date of tenant agreement.",
    )
    agreement_duration = fields.Integer("Agreement Duration", index=True, tracking=2)
    agreement_duration_type = fields.Selection(
        [("month", "Month"), ("year", "year"), ("one_time", "One Time")],
    ...
    agreement_expiry_date = fields.Date(
        string="Agreement Expire On",
        copy=False,
    ...
    property_id = fields.Many2one(
        ...
        "Unidad",
        required=True,
        domain="[('is_property','=', True),('state','=', 'available')]",
        index=True,
        tracking=4,
    )
    ...
    property_rent = fields.Float(
        "Rent", related="property_id.property_rent_price", store=True
    )
    property_sale_price = fields.Float(
        "Precio de venta", related="property_id.property_sale_price", store=True
    )
    ...
    commission_type = fields.Selection(
        ...
        related="property_id.property_agent_commission_type",
        store=True,
    )
    agent_commission = fields.Float(
        "Commission",
        related="property_id.property_agent_commission",
        currency_field="currency_id",
        store=True,
    )
    landloard_id = fields.Many2one(
        "res.partner", related="property_id.property_landlord_id", store=True
    )
    ...
    total_maintenance = fields.Monetary(
        "Total Maintenance", compute="_compute_amount_all", store=True
    )
    maintenance_charge = fields.Float(
        "Maintenance Charge",
        related="property_id.property_maintenance_charge",
        currency_field="currency_id",
        store=True,
    )
    maintenance_interval_type = fields.Selection(
        [("month", "Monthly"), ("year", "Yearly"), ("one_time", "One Time")],
        string="Maintenance Interval ",
        related="property_id.property_maintenance_interval_type",
        store=True,
    )
    tenant_id = fields.Many2one("res.partner", string="Tenant", required=True)
    payment_option = fields.Selection(
        [("single", "Single Payment"), ("installment", "Installments")],
        string="Payments Option",
        default="installment",
    )
    ...
    reserve_amount = fields.Float(
        "Monto De Reserva", currency_field="currency_id", store=True
    )
    initial_amount = fields.Float(
        "Monto de Separación", currency_field="currency_id", store=True
    )
    currency_id = fields.Many2one(
        "res.currency",
        string="Moneda",
    ...
    amount_to_finance = fields.Float(
        "Monto Inicial", currency_field="currency_id", store=True
    )
    gastos_legales = fields.Float(
        "Gastos Legales", currency_field="currency_id", store=True, default=500.0
    )
    gastos_legales_invoiced = fields.Boolean("Gastos Legales Invoiced", default=False)
    first_installment_date = fields.Date(
        string="Pago de primer cuota", copy=False, store=True
    )
    co_tenant_id = fields.Many2one("res.partner", string="Copropietario")
    delivery_date = fields.Date(
        "Fecha de entrega", store=True, related="property_id.delivery_date"
    )
    ...
    co_owner_relationship = fields.Selection(
        ...
        string="Relación con el Co-propietario",
        help="Select the relationship type between the tenant and the co-owner.",
    )
    financing_amount = fields.Float(
        compute="_compute_financing_details", string="Financiamiento", store=True
    )

    financed_percentage = fields.Float(
        compute="_compute_financing_details", string="Porcentaje Financiado", store=True
    )
    ...
    @api.depends(
        "property_sale_price", "initial_amount", "reserve_amount", "amount_to_finance"
    )
    def _compute_financing_details(self):
        for record in self:
            # Calculate the financing amount
            record.financing_amount = record.property_sale_price - (
                record.initial_amount + record.amount_to_finance
            )
            ...
            if record.property_sale_price and record.property_sale_price != 0:
                record.financed_percentage = (
                    float(record.financing_amount)
                    / float(record.property_sale_price)
                    * 100.0
                )
            else:
    ...
    def _compute_formatted_financed_percentage(self):
        for record in self:
            if record.property_sale_price and record.property_sale_price != 0:
                ...
                financed_percentage = (
                    float(record.property_sale_price - computed_paid_amount)
                    / float(record.property_sale_price)
                    * 100.0
                )
                ...
                if len(percentage_str) > 2:
                    record.formatted_financed_percentage = (
                        percentage_str[:2] + "" + percentage_str[2:]
                    )
                else:
                            ...
                            "invoice_line_ids": [
                                (
                                    ...
                                    0,
                                    {
                                        ...
                                        "quantity": 1,
                                        "price_unit": self.total_price
                                        / self.partial_payment_id.number_of_installments,
                                        "account_id": accounts["income"].id,
                                    },
                                ),
                                (
        ...
        if self.payment_option == "single":
            ...
            if self.partial_payment_id.is_custom:
                ...
                def months_between_dates(initial_date_str, final_date_str):
                    ...
                    month_diff = (
                        (final_date.year - initial_date.year) * 12
                        + final_date.month
                        - initial_date.month
                    )
                ...
                months_until_last_installment = months_between_dates(
                    self.first_installment_date, self.delivery_date
                )

                regular_installment_amount = (
                    self.amount_to_finance / months_until_last_installment
                )
                allocated_amount = 0
        ...
        self.write(
            {
                "state": "confirm",
                "name": self.env["ir.sequence"].next_by_code(
                    "tenancy.agreement.sequence",
                    sequence_date=fields.Datetime.context_timestamp(
                        self, fields.Datetime.to_datetime(self.agreement_date)
                    ),
                ),
            }
        )
    ...
    def check_tenancy_agreement_validity(self):
        start_agreement = self.search(
            [
                ("agreement_start_date", "=", datetime.datetime.today().date()),
                ("state", "=", "confirm"),
                ("property_type", "=", "rent"),
            ]
        )
        expiry_agreement = self.search(
            [
                ("agreement_expiry_date", "=", datetime.datetime.today().date()),
                ("state", "=", "running"),
            ]
        )
        if expiry_agreement:
            for record in expiry_agreement:
                ...
                record.property_id.write(
                    {"state": "available", "current_user_id": False}
                )
        if start_agreement:
            for record in start_agreement:
                ...
                if record.property_type == "rent":
                    ...
                    for i in range(0, month):
                        ...
                        inv_id = self.env["account.move"].create(
                            {
                                ...
                                "invoice_date": record.agreement_start_date
                                + relativedelta(months=record.agreement_duration),
                                "is_property_invoice": True,
                        ...
                        accounts = (
                            record.property_id.product_tmpl_id.get_product_accounts(
